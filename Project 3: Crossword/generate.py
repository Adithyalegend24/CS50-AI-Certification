...
class CrosswordCreator():
    ...
    def backtrack(self, assignment):
        """
        Solve the crossword using backtracking search.
        Returns a complete assignment if one exists, otherwise None.
        """

        # If all variables are assigned, solution is found
        ...
        # Try each possible value for the chosen variable
        for word in self.order_domain_values(variable, assignment):

            # Assign value temporarily
            assignment[variable] = word

            # Check consistency before continuing
            if self.consistent(assignment):

                # Maintain arc consistency
                self.ac3()

                # Continue with recursive backtracking
